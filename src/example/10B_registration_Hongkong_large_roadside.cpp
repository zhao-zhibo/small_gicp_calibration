...
void CalibrateLiDAR(const std::vector<Eigen::Vector4f>& target_points, const std::vector<Eigen::Vector4f>& source_points) {
  ...
  T_roadLidar_Vehiclelidar <<
    -0.310384571552,  0.496996223927, -0.810343205929, -36.100292205811,
     0.183733299375,  0.867734968662,  0.461820393801, -38.156749725342,
     0.932686150074, -0.005545103457, -0.360646307468,   5.387669086456,
     0.0,             0.0,             0.0,              1.0;
  // // 迭代之后的结果如下:
  //    [-0.788217, -0.615396, 0.00126103, 525.155,
  //                0.615397, -0.788217, 0.000295431, -374.261,
  //                0.000812155, 0.0010089, 0.999999, -0.414991,
  //                0.0, 0.0, 0.0, 1.0]
...
int main(int argc, char** argv) {
  ...
  pcl::io::loadPCDFile("/home/zhao/Data/large_roadside_pcd/global_part_0.1_part.pcd", *vehicleLiDARPoints); // 点云的pcd路径
  
  // 2. source  路侧点云,1681274984这个时间戳的点云作为香港的路侧对应的点云帧，这一帧点云没有什么障碍物
  pcl::PointCloud<pcl::PointXYZ>::Ptr roadLiDARPoints(new pcl::PointCloud<pcl::PointXYZ>());
  pcl::io::loadPCDFile("/home/zhao/Data/large_roadside_pcd/39762808.918863000.pcd", *roadLiDARPoints); // 高精地图的pcd路径
  ...
  std::vector<Eigen::Vector4f> source_points = convertToEigenPoints(roadLiDARPoints);
  // 将 roadLiDARPoints 转换为 target_points
  std::vector<Eigen::Vector4f> target_points = convertToEigenPoints(vehicleLiDARPoints);
