...
void CalibrateLiDAR(const std::vector<Eigen::Vector4f>& target_points, const std::vector<Eigen::Vector4f>& source_points) {
  ...
  // // 下面这个是车载lidar运行的结果和路侧lidar标定结果计算出来的结果，不是很准确
  // T_roadLidar_Vehiclelidar <<
  //     0.811046,   -0.582558,   -0.0532023,   12.623,
  //     0.581948,    0.812747,   -0.0279182,  -31.9255,
  //     0.059504,   -0.00831803,  0.998194,   -23.8976,
  //     0.0,         0.0,         0.0,          1.0;
  // // 下面这个用车载的真值结果和路侧lidar的标定结果，然后给计算出来的结果
  T_roadLidar_Vehiclelidar <<
        0.861211,   -0.508174,   0.008592,   13.949491,
        0.508156,   0.860613,   -0.033524,   -9.501258,
        0.009642,   0.033238,   0.999401,   0.435950,
...
int main(int argc, char** argv) {
  ...
  pcl::io::loadPCDFile("/home/zhao/Data/HK_data/Temp_1681274991_755/1681274991.8028_roadSide.pcd", *roadLiDARPoint); // 路侧点云的pcd路径

  // 2. source  车载点云
  pcl::PointCloud<pcl::PointXYZ>::Ptr vehileLiDARPoint(new pcl::PointCloud<pcl::PointXYZ>());
  pcl::io::loadPCDFile("/home/zhao/Data/HK_data/Temp_1681274991_755/1681274991.7552_VehicleleCloud.pcd", *vehileLiDARPoint); // 车载点云的pcd路径
