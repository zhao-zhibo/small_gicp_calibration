...
void CalibrateLiDAR(const std::vector<Eigen::Vector4f>& target_points, const std::vector<Eigen::Vector4f>& source_points) {
  ...
  // 下面这个是用的车载系统的初值和路侧的标定结果一起算出来的
  // T_roadLidar_Vehiclelidar <<
  //   0.911179,   -0.404,     -0.0808469,   18.6541,
  //   0.403883,    0.914623,  -0.0185274,  -41.5733,
  //   0.0814295,  -0.0157709,  0.996554,   -19.3381,
  //   0.0,         0.0,        0.0,         1.0;

  // 下面这个是用车载系统的真值和路侧标定的结果给算出来的
  T_roadLidar_Vehiclelidar <<
      0.945352,   -0.325376,   -0.020989,   19.549219,
      0.324707,   0.945340,   -0.029968,   -22.507219,
      0.029593,   0.021515,   0.999331,   0.408989,
      0.000000,   0.000000,   0.000000,   1.000000;

// 下面这个是用于路侧和车载的初值使用，1681274995.4584车载pcd文件
// [INFO] [1770435703.809691487]: [Sync Time met]: roadCloudTime = 1681274995.4903 s, keyframeTimestamp = 1681274995.4584 s
// [INFO] [1770435703.809703096]: [RoadCloudQueue size]: 2 | hasSynced: 1 | keyframeTimestamp: 1681274995.45837
// [INFO] [1770435703.809713503]: [RoadCloudQueue timestamps]: [head 0]: 1681274995.646419 | [head 1]: 1681274995.782765 |  || [tail 0]: 1681274995.782765 | [tail 1]: 1681274995.646419 | 
// [DEBUG] Initial Transform Matrix (Road to Vehicle):
//    0.998519   0.0465218  -0.0281972     5.34372
//  -0.0467888    0.998866 -0.00888137     9.03818
//    0.027752   0.0101875    0.999563    -7.19117
//           0           0           0           1
...
int main(int argc, char** argv) {
  ...
  pcl::io::loadPCDFile("/home/zhao/Data/HK_data/Temp/1681274989.6723_roadSide.pcd", *roadLiDARPoint); // 路侧点云的pcd路径

  // 2. source  车载点云
  pcl::PointCloud<pcl::PointXYZ>::Ptr vehileLiDARPoint(new pcl::PointCloud<pcl::PointXYZ>());
  pcl::io::loadPCDFile("/home/zhao/Data/HK_data/Temp/1681274989.6534_VehicleleCloud.pcd", *vehileLiDARPoint); // 车载点云的pcd路径

  std::cout << "Loaded roadLiDAR with " << roadLiDARPoint->width * roadLiDARPoint->height << " data points." << std::endl;
  std::cout << "Loaded vehileLiDARPoint with " << vehileLiDARPoint->width * vehileLiDARPoint->height << " data points." << std::endl;
